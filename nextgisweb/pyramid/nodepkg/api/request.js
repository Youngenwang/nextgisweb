...
export async function request(path, options) {
    ...
    const respJSON = respCType && (
        ...
        respCType.includes("application/vnd.lunkwill+json")
    );
...
async function handleLunkwillResponse(lwResp) {
    ...
    if (ct === undefined || !ct.includes("application/vnd.lunkwill+json")) {
        return lwResp;
    }

    let lwData = await responseJson(lwResp);
    let delay = lwData.delay;
    const retry = lwData.retry !== undefined ? lwData.retry : 2000;
